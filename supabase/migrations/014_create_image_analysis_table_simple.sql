...
-- This is a simplified version without foreign key constraints for testing
...
CREATE TABLE image_analysis (
    ...
    test_id UUID NOT NULL, -- Will add foreign key constraint later
...
CREATE INDEX idx_image_analysis_test_id ON image_analysis(test_id);
CREATE INDEX idx_image_analysis_power_mode ON image_analysis(power_mode);
CREATE INDEX idx_image_analysis_image_index ON image_analysis(image_index);
CREATE INDEX idx_image_analysis_analyzed_at ON image_analysis(analyzed_at);

-- Add comments for documentation
COMMENT ON TABLE image_analysis IS 'Stores AI analysis results for each individual LPF/HPF image';
COMMENT ON COLUMN image_analysis.test_id IS 'Reference to urine_tests table (foreign key will be added later)';
...
COMMENT ON COLUMN image_analysis.confidence IS 'AI confidence percentage for analysis (0-100.00)';
